...
class ClickerImport implements  ToCollection, WithHeadingRow, WithChunkReading
{
    protected $fileName;
    use Logger;
        ...
        foreach ($rows as $row) {
            ...
            if (!$productTitle) {
                $error = ValidationException::withMessages([
                    'name' => ['Please add' . ' "' . $row['item_name'] . '" ' . ' into Product title table because this product name is not matching any lable. The file name is ' . $str . ' and order number is ' . $row['order_number']]
                ]);
                throw $error;
            }

            $moduleName = Clicker::updateOrCreate(
