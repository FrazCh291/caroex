...
class XstreamgymImport implements ToCollection, WithHeadingRow, WithChunkReading
{
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $row) {
            ...
            $ship_first_name = $row['first_name_shipping'] == null ? null : Str::of($row['first_name_shipping'])->trim();
            $ship_last_name = $row['last_name_shipping'] == null ? null : Str::of($row['last_name_shipping'])->trim();
            $customer_name = $ship_first_name . "  " . $ship_last_name;
            ...
            $data = Xstreamgym::updateOrCreate(
                ['order_number' => $row['order_number'],
                    'item_name' => $row['item_name']],
                [
                    'company_id' => Auth::user() ? Auth::user()->company_id : '',
                    'product_id' => $productTitle ? $productTitle['id'] : null,
                    ...
                    'order_status' => $row['order_status'] == null ? null : Str::lower(Str::of($row['order_status'])->trim()),
                    ...
                    'billing_state_code' => $row['state_code_billing'] == null ? null : Str::of($row['state_code_billing'])->trim(),
...
//                    'order_date' => $date ? $date : "",
                    ...
                    'billing_customer_name' => $row['first_name_shipping'] == null ? null : Str::of($row['first_name_shipping'])->trim(),
                    'billing_company' => $row['company_billing'] == null ? null : Str::of($row['company_billing'])->trim(),
                    'billing_address_1' => $row['address_12_billing'] == null ? null : Str::of($row['address_12_billing'])->trim(),
                    'billing_address_2' => '',
                    'billing_city' => $row['city_billing'] == null ? null : Str::of($row['city_billing'])->trim(),
                    ...
                    'shipping_customer_name' => $customer_name ? $customer_name : '',
                    'shipping_address_1' => $row['address_12_shipping'] == null ? null : Str::of($row['address_12_shipping'])->trim(),
                    'shipping_address_town' => $row['city_shipping'] == null ? null : Str::of($row['city_shipping'])->trim(),
                    'shipping_address_postcode' => $row['postcode_shipping'] == null ? null : Str::of($row['postcode_shipping'])->trim(),
                    'shipping_country' => $row['country_code_shipping'] == null ? null : Str::of($row['country_code_shipping'])->trim(),
            ...
            $orderItem = OrderItem::updateOrCreate(
                ...
                ['order_id' => $order->id,
                    ...
                    'product_name' => $row['item_name'] == null ? null : Str::of($row['item_name'])->trim(),
                    'item_cost' => $row['item_cost'] == null ? null : Str::of($row['item_cost'])->trim(),
                    'quantity' => ($row['quantity'] == null ? null : (Str::of($row['quantity'])->trim() ? Str::of($row['quantity'])->trim() : 1)),
                    'order_date' => trim($row['order_date']) == 'NULL' || trim($row['order_date']) == '' ? null : $date,
                ]);
            ...
            $invoice = $order->invoices()->create(
                ...
                'invoice_date' => trim($row['order_date']) == 'NULL' || trim($row['order_date']) == '' ? null : $date,
                'currency' => 'gbp',
            ...
            $payment->payment_reference = $row['order_number'] == null ? 0 : Str::of($row['order_number'])->trim();
            $payment->payment_method = 'payment_method_card';
            $payment->amount = $invoice ? $invoice->total : '';
            $payment->payee_total = $invoice ? $invoice->total : '';
            $payment->payment_date = trim($row['order_date']) == 'NULL' || trim($row['order_date']) == '' ? null : $date;
            $payment->save();

            $warehouse = Company::where('name', 'Bacup')->orderby('id', 'desc')->first();

            $companyId = Auth::user() ? Auth::user()->company_id : '';
...
//            ]);
           ...
           $product = Product::where('id', $productTitle->product_id)->first();
            ...
            if ($product->shipping_method && $warehouse) {
                        ...
                        $stockLogs->company_id = $companyId;
                        $stockLogs->order_item_id = $orderItem->id;
                        $stockLogs->warehouse_id = $warehouse ? $warehouse->id : 1;
                        $stockLogs->supplier_id = 1;
                        $stockLogs->user_id = Auth::user() ? Auth::user()->id : '';
                        $stockLogs->product_id = $productTitle ? $productTitle->product_id : null;
                        $stockLogs->quantity = '-' . $orderItem->quantity;
                        $stockLogs->save();
