...
// dd($stockLogs->get());
        ...
        $stockLogs = $stockLogs->with(['supplier', 'wareHouse', 'product', 'orderItem', 'user', 'company'])->orderBy('created_at', 'desc')->paginate(10);
    ...
    public function create()
    {
        ...
        $warehouses = Company::Select('name', 'id')->where('parent_id', Auth::user()->company_id)->where('type', 'warehouse')->get();


        return Inertia::render('StockLogs/Create', [
            'products' => $products,
            'suppliers' => $suppliers,
    ...
    public function store(Request $request)
    {
        ...
        $stockLogs->company_id = Auth::user()?Auth::user()->company_id:'';
        ...
        $this->logable($module);
        $productStock = ProductStock::where(['product_id', $request->product_id[0]])->where('company_id', Auth::user()->company_id)->first();
        if($productStock) {
            $product_Stock = $productStock->where('date', date("Y-m-d", strtotime(Carbon::today())))->first();
            if ($product_Stock) {
                $product_Stock->quantity = intVal($product_Stock->quantity) + intVal($request->quantity);
                $product_Stock->date = date("Y-m-d", strtotime(Carbon::today()));
                $product_Stock->save();

                $currentURL = url()->current();
                $slice = Str::after($currentURL, '8000');
                $module = [
                    'message' => '{' . $product_Stock->id  .'}',
                ...
                $product_Stock->company_id = Auth::user()?Auth::user()->company_id:'';
                $product_Stock->product_id = $request->product_id[0];
                $product_Stock->quantity = intVal($productStock->quantity) + intVal($request->quantity);
                $product_Stock->date = date("Y-m-d", strtotime(Carbon::today()));
                $product_Stock->save();
        ...
        return redirect(route('stock-logs.index'))->with('success', 'Product title created successfully');
    }
    ...
    {
        $this->authorize('update', StockLog::class);
    ...
    public function update(Request $request, $id)
    {
        ...
        $productStock = ProductStock::where('product_id', $request->value[0])->where(
            'company_id',
            Auth::user()->company_id
        )->where('date', date("Y-m-d", strtotime(Carbon::today())))->first();
        if ($productStock) {
            ...
            $productLastQuantity = ProductStock::where('product_id', $request->value[0])->orderBy(
                'date',
                'desc'
            )->where('date', '<', date("Y-m-d", strtotime(Carbon::today())))->first();
            $productStock = new ProductStock();
    ...
    {
        $this->authorize('delete', StockLog::class);

        $stockLogs = StockLog::findOrFail($id);
        $warehouseStock = WarehouseStock::where([['warehouse_id',$stockLogs->warehouse_id],['product_id',$stockLogs->product_id]])->first();
        $warehouseStock->quantity = $warehouseStock->quantity - ($stockLogs->quantity);
        $warehouseStock->update();
        $productStock = ProductStock::where('product_id',$stockLogs->product_id)->first();
        $productStock->quantity = $productStock->quantity - ($stockLogs->quantity);
        $productStock->update();
        $stockLogs->delete();
    ...
    public function reportCreate(Request $request)
    {
        ...
        if ($data) {
            ...
            $stockLogs = $stockLogs->whereBetween('created_at', [date(
                "Y-m-d 00:00:00",
                strtotime(Carbon::today()->subWeek())
            ), date(
                "Y-m-d 23:59:59",
            ...
            $query->where(function ($query) use ($filters) {
                if (isset($filters['product_id']) && !isset($filters['warehouse_id']) && !isset($filters['supplier_id'])) {
                    if ($filters['product_id'] === 'all') {
                        ...
                        $query->where('product_id', $filters['product_id'])->whereBetween(
                            'created_at',
                            [
                ...
                if (!isset($filters['product_id']) && isset($filters['warehouse_id']) && !isset($filters['supplier_id'])) {
                    if ($filters['warehouse_id'] === 'all') {
                        ...
                        $query->where('warehouse_id', $filters['warehouse_id'])->whereBetween(
                            'created_at',
                            [
                                date("Y-m-d 00:00:00", strtotime($filters['date_from'])),
                                date("Y-m-d 23:59:59", strtotime($filters['date_to']))
                ...
                if (!isset($filters['product_id']) && !isset($filters['warehouse_id']) && isset($filters['supplier_id'])) {
                    if ($filters['supplier_id'] === 'all') {
                        ...
                        $query->where('supplier_id', $filters['supplier_id'])->whereBetween(
                            'created_at',
                            [
                                date("Y-m-d 00:00:00", strtotime($filters['date_from'])),
                                date("Y-m-d 23:59:59", strtotime($filters['date_to']))
                ...
                if (isset($filters['product_id']) && isset($filters['warehouse_id']) && !isset($filters['supplier_id'])) {
                    if ($filters['product_id'] === 'all' && $filters['warehouse_id'] === 'all') {
                        ...
                        $query->where('product_id', $filters['product_id'])->where(
                            'warehouse_id',
                ...
                if (!isset($filters['product_id']) && isset($filters['warehouse_id']) && isset($filters['supplier_id'])) {
                    if ($filters['warehouse_id'] === 'all' && $filters['supplier_id'] === 'all') {
                        ...
                        $query->where('warehouse_id', $filters['warehouse_id'])->where(
                            'supplier_id',
                            $filters['supplier_id']
                        )->whereBetween('created_at', [date(
                            "Y-m-d 00:00:00",
    ...
    public function stockReportExportPDF(Request $request)
    {
        ...
        if ($request['product_id'] !== null && $request['warehouse_id'] == null && $request['supplier_id'] == null) {
            if ($request['product_id'] === 'all') {
                ...
                $stockLogs = StockLog::whereBetween('created_at', [date(
                    "Y-m-d 00:00:00",
                ...
                $stockLogs = StockLog::where('product_id', $request['product_id'])->whereBetween(
                    'created_at',
                    [date("Y-m-d 00:00:00", strtotime($request['date_from'])), date(
                        "Y-m-d 23:59:59",
        ...
        if ($request['product_id'] == null && $request['warehouse_id'] !== null && $request['supplier_id'] == null) {
            if ($request['warehouse_id'] === 'all') {
                ...
                $stockLogs = StockLog::where('warehouse_id', $request['warehouse_id'])->whereBetween(
                    'created_at',
                    [date("Y-m-d 00:00:00", strtotime($request['date_from'])), date(
                        "Y-m-d 23:59:59",
                        strtotime($request['date_to'])
        ...
        if ($request['product_id'] == null && $request['warehouse_id'] == null && $request['supplier_id'] !== null) {
            if ($request['supplier_id'] === 'all') {
                ...
                $stockLogs = StockLog::where('supplier_id', $request['supplier_id'])->whereBetween(
                    'created_at',
                    [date("Y-m-d 00:00:00", strtotime($request['date_from'])), date(
                        "Y-m-d 23:59:59",
                        strtotime($request['date_to'])
        ...
        if ($request['product_id'] !== null && $request['warehouse_id'] !== null && $request['supplier_id'] == null) {
            if ($request['product_id'] === 'all' && $request['warehouse_id'] === 'all') {
                ...
                $stockLogs = StockLog::where('product_id', $request['product_id'])->where(
                    'warehouse_id',
        ...
        if ($request['product_id'] == null && $request['warehouse_id'] !== null && $request['supplier_id'] !== null) {
            if ($request['warehouse_id'] === 'all' && $request['supplier_id'] === 'all') {
                ...
                $stockLogs = StockLog::where('warehouse_id', $request['warehouse_id'])->where(
                    'supplier_id',
                    $request['supplier_id']
                )->whereBetween('created_at', [date(
                    "Y-m-d 00:00:00",
        ...
        if ($request['product_id'] !== null && $request['warehouse_id'] !== null && $request['supplier_id'] !== null) {
            if ($request['product_id'] === 'all' && $request['warehouse_id'] === 'all' && $request['supplier_id'] === 'all') {
                ...
                $stockLogs = StockLog::where('product_id', $request['product_id'])->where(
                    'warehouse_id',
                    $request['warehouse_id']
                )->where('supplier_id', $request['supplier_id'])->whereBetween(
                    'created_at',
                    [date("Y-m-d 00:00:00", strtotime($request['date_from'])), date(
                        "Y-m-d 23:59:59",
                        strtotime($request['date_to'])
