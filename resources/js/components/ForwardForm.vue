<template>
    <div id="forwardModal" aria-hidden="true" aria-labelledby="myModalLabel33" class="modal fade text-left" role="dialog" tabindex="-1">
            ...
            <div class="modal-content">
                ...
                <form class="form form-horizontal">
                        <div class="modal-body mb-1">
                            ...
                            <div class="form-group py-0 my-0 mb-1">
                                <label>Body *</label>
                                <div id="formBody" contenteditable="true"  v-html="form.body" style="width: 100%; min-height: 300px; padding: 5px; border: 1px solid #e9e9e9; border-radius: 5px; outline: none;">
                                </div>
                            </div>
                            <div class="form-group py-0 my-0 mb-1">
                                <input id="forwardfilesUploaded" type="file" ref="myFileInput" multiple @input="form.uploadAttachments = $event.target.files"
                                    @change="uploadImage" accept="image/*">
                            </div>
                            <div class="form-group py-0 my-0" v-if="form.attachments.length>0">
                                <div class="color" v-for="(attachment, key) in form.attachments" :key="key">
                                    <div class="test br">
                                        {{ attachment.name }}
                                    </div>
                                </div>
...
export default {
    ...
    mounted() {
        if(this.form.body) {
            ...
            if(this.form.cc) {
                this.email.cc.forEach(function(singleCc, index){
                    if(index===0) {
                        body = body.concat("<br><strong>Cc:</strong> &lt;"+singleCc.mail);                
                    }
                    else {
                        body = body.concat(", "+singleCc.mail); 
                    }
                });
                body = body.concat("&gt;");
            }
            if(this.form.bcc) {
                this.email.bcc.forEach(function(singleBcc, index){
                    if(index===0) {
                        body = body.concat("<br><strong>Cc:</strong> &lt;"+singleBcc.mail);                
                    }
                    else {
                        body = body.concat(", "+singleBcc.mail); 
                    }
                });
                body = body.concat("&gt;");
            }
        ...
        if(this.form.subject) {
            if(this.form.subject.slice(0, 3).toLowerCase() === 'fw:') {
                this.form.subject = "FW: "+this.form.subject.substring('3').trim();
            }
            else {
                this.form.subject = "FW: "+this.form.subject;
            }
        }
    ...
    methods: {
        ...
        submit() {
            if(!this.form.from){
                ...
                this.form.body = document.getElementById('formBody').innerHTML;
                this.form.post(route('email.forward'));
                this.resetInput();
            }
        },
        resetInput(index) {
        ...
        removeImage(index) {
            var attachments = document.getElementById("forwardfilesUploaded").files;
            var fileBuffer = new DataTransfer();
            for (let i = 0; i < attachments.length; i++) {
                if (index !== i)
                    fileBuffer.items.add(attachments[i]);
            }
            document.getElementById("forwardfilesUploaded").files = fileBuffer.files;
            this.form.uploadAttachments = fileBuffer.files;
            this.images.splice(index, 1);
        },

        chooseFiles: function () {
            document.getElementById("forwardfilesUploaded").click()
        },
    },
}
</script>
