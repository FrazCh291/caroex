                                    ...
                                    <form @submit.prevent="submit" class="form form-horizontal">
                                        <div class="form-body">
                                            ...
                                            <div class="col-sm-11 d-flex justify-content-start px-0">
                                                ...
                                                <inertia-link :href="route('suppliers.show', supplier.id)" type="button"
                                                              class="btn btn-light-secondary mr-1 mb-1" v-if="supplier">
                                                    Back
                                                </inertia-link>
                                                <inertia-link :href="route('suppliers.show', document.documentable_id)"
                                                              type="button"
...
export default {
    ...
    beforeMount() {
        document.title = process.env.MIX_APP_NAME + " - Create suppliers Documents";
        if (this.document) {
            this.update = true;
    ...
    methods: {
        submit() {
            if (this.update) {
                this.form.post(route('suppliers.document', this.document.id))
                this.update = false;
            } else {
                this.form.supplier_id = this.supplier.id;
                this.form.post('/suppliers/files/store', this.supplier_id);
            }
        },
