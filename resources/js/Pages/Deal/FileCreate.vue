...
export default {
    name: "Create",
    props: ['deal', 'document', 'dealNumber'],
    components: {
        Label,
        AdminLayout,
        JetInputError,
    ...
    beforeMount() {
        document.title = process.env.MIX_APP_NAME + " - Create Deal Documents";
        if (this.document) {
            this.update = true;
    ...
    methods: {
        submit() {
            if (this.update) {
                this.form.post(route('deals.document', this.document.id))
                this.update = false;
            } else {
                this.form.deal_id = this.deal.id;
                this.form.post('/erp/deals/files/store', this.deal_id);
            }
        },
