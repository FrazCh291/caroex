                    ...
                    <div class="col-md-9 col-12">
                        <div class="card">
                            <div class="card-header pb-0 mb-0">
                                <h4 class="card-title" v-if="customers">Edit Review</h4>
                                <h4 class="card-title" v-else>Add Review</h4>
                            </div>
                            <div class="px-2 pb-1">
                                <hr class="line">
                            </div>
...
export default {
    name: "Create",
    props: ['review', 'products','review_statuses', 'salesChannels', 'customer', 'currentDate', 'customer_id','review', 'errors'],
    components: {
        Label,
        moment,
        AdminLayout,
    ...
    beforeMount() {
        document.title = process.env.MIX_APP_NAME + " - Create Review";
        if (this.review) {
            this.update = true;
    ...
    methods: {
        submit() {
            if (this.update) {
                ...
                this.form.post(route('customer.reviews.update', this.review.id))
            } else {
                if(this.form.status === '') {
                    this.form.status = 'review_unapproved';
                }
                this.form.customer_id = this.customer_id                            
                 this.form.post('/erp/customer/{id}/reviews/store' , this.customer_id)
            }
        },
    }
}
