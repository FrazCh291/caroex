<template>
  <admin-layout>
                          ...
                          <div class="text-base pt-1">
                            ...
                            <p class="tag">
                              <input
                                ...
                                v-on:click="check_one()"
                              />
                              <label class="pl-1 py-0 my-0" for="description"
                                >Description</label
                              >
                            </p>
                          </div>
          ...
          <div class="card-content">
            <div class="table-responsive">
              <table class="table table-hover mb-0">
                ...
                <tbody>
                  <tr v-for="quote in quotation.data">
                    <td class="text-truncate py-0 my-0 text-left">
                      {{ quote.supplier ? quote.supplier.name : "" }}
                    </td>
                    ...
                    <td class="text-right py-0 my-0">
                      <span class="d-inline-flex align-items-center">
                        <inertia-link
                          :href="route('quotationItem.show', quote.id)"
                        >
                          <span
                            class="
                        ...
                        <button v-on:click="confirmDelete(quote.id)">
                          <span
    ...
    <div class="col-12">
      <pagination :links="quotation.links" class="float-right"></pagination>
    </div>
  </admin-layout>
</template>
...
export default {
  name: "index",
  props: ["quotation", "quotations", "params"],
  components: {
    Button,
    AdminLayout,
    Pagination,
  ...
  beforeMount() {
    document.title = process.env.MIX_APP_NAME + "  - Quotation";
  },
  mounted() {
    if (this.params) {
  ...
  methods: {
    ...
    deleteItem() {
      this.sweetAlert = false;
      this.$inertia.delete(`/fulfilment/admin/quotations/${this.itemId}`);
    },
    confirmDelete(id) {
    ...
    check_one: function () {
      ...
      if ((this.query.description = false)) {
        this.query.description = true;
        this.query.supplier_id = [];
      }
    },
    loadData() {
