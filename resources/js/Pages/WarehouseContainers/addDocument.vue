                    ...
                    <div class="col-md-9 col-12">
                        <div class="card">
                            <div class="card-header pb-0 mb-0">
                                <h4 class="card-title" v-if="document">Edit Container Document</h4>
                                <h4 class="card-title" v-else>Add Container Document</h4>
                            </div>
                            <div class="px-2 pb-1">
                                <hr class="line">
                            </div>
                                ...
                                <div class="card-body">
                                    <form @submit.prevent="submit" class="form form-horizontal">
                                        ...
                                        <div class="col-sm-11 d-flex justify-content-start px-0">
                                            ...
                                            <inertia-link
                                                :href="route('containers.show', document.documentable_id)"
                                                type="button"
...
export default {
    name: "addDocument",
    props: ['warehouseContainer', 'errors', 'document'],
    components: {
        Label,
        AdminLayout,
        JetInputError,
    ...
    beforeMount() {
        document.title = process.env.MIX_APP_NAME + " - Create Container Documents";
        if (this.document) {
            this.update = true;
    ...
    methods: {
        submit() {
            if (this.update) {
                this.form.post(route('container.document.update', this.document.id))
            } else {
                this.form.warehouseContainer_id = this.warehouseContainer.id;
                this.form.post('/fulfilment/containers/{id}/files/store',this.warehouseContainer.id);
            }
        },
    }
}
