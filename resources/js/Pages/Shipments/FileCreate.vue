                                    ...
                                    <form @submit.prevent="submit" class="form form-horizontal">
                                        <div class="form-body">
                                            ...
                                            <div class="col-sm-11 d-flex justify-content-start px-0">
                                                ...
                                                <inertia-link :href="route('shipments.show', shipment.id)"
...
export default {
    ...
    beforeMount() {
        document.title = process.env.MIX_APP_NAME + " - Create Shippments Documents";
        if (this.document) {
            this.update = true;
    ...
    methods: {
        submit() {
            if (this.update) {
                this.form.post(route('update.document', this.document.id))
                this.update = false;
            } else {
                this.form.shippment_id = this.shipment.id;
                this.form.post('/shipments/files/store', this.shipment_id);
            }
        },
