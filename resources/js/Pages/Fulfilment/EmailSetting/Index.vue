    ...
    <admin-layout>
        <div class="col-12 px-0">
            <div class="col form-group p-0">
                <inertia-link :href="route('email-setting.create')" class="btn btn-primary" data-repeater-create="">
                    Add Emails
                </inertia-link>
            </div>
        </div>
        <div class="row pb-3" id="table-hover-row">
                                                    ...
                                                    <div class="text-base pt-1">
                                                        <p class="tag">
                                                            <input type="checkbox" name="username" id="username"
                                                                   v-model="query.username" v-on:click="check_one()">
                                                            <label class="pl-1 py-0 my-0" for="username">Username</label>
                                                        </p>
                                                        <p class="tag">
                                                            <input type="checkbox" name="channel_id" id="channel_id"
                                                                   v-model="query.channel_id" v-on:click="check_one()">
                                                            <label class="pl-1 py-0 my-0" for="channel_id">channel</label>
                                                        </p>
                                                        <p class="tag">
                                                            <input type="checkbox" name="protocol" id="protocol"
                                                                   v-model="query.protocol" v-on:click="check_one()">
                                                            <label class="pl-1 py-0 my-0" for="protocol">protocol</label>
                                                        </p>
                                                        ...
                                                        <!-- <p class="tag">
                                                            <input type="checkbox" name="host" id="host"
                                                                   v-model="query.host">
                                                            <label
                                                                class="pl-1 py-0 my-0" for="host">Host</label>
                                                        </p> -->
                                                        <!-- <p class="tag">
                                                            <input type="checkbox" name="port" id="port"
                                                                   v-model="query.port">
                                                            <label class="pl-1 py-0 my-0" for="port">Port</label>
                                                        </p>
                                                        <p class="tag">
                                                            <input type="checkbox" name="encryption" id="encryption"
                                                                   v-model="query.encryption">
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover mb-0">
                                ...
                                <tr>
                                    ...
                                    <th class="text-left pl-2">Channel</th>
                                    <th>Protocol</th>
                                ...
                                <tbody>
                                <tr v-for="email in emails.data">
                                    <td class="text-left pl-2">{{ email.username }}</td>
                                    <td class="text-left pl-2">
                                        {{ email.email_account ? email.email_account.name : '' }}
                                    </td>
                                    <td>{{ email.protocol }}</td>

                                    <td class="text-center">
                                        <span v-if="email.status == 1" class="badge badge-light-success badge-pill">Active</span>
                                    ...
                                    <td class="text-right">
                                            <span class="d-inline-flex align-items-center">
                                                <inertia-link :href="route('email-settings.edit',email.id)">
                                                ...
                                                <button v-on:click="confirmDelete(email.id)">
            ...
            <div class="col-12 ">
                <pagination :links="emails.links" class="float-right"></pagination>
            </div>
        </div>
    </admin-layout>
</template>
...
export default {
    ...
    beforeMount() {
        document.title = process.env.MIX_APP_NAME + " - Email setting";
    },
    mounted() {
        if (this.params) {
    ...
    methods: {
        ...
        deleteItem() {
            this.sweetAlert = false
            this.$inertia.delete(`/fulfilment/admin/email-setting/${this.itemId}`)
        },
        confirmDelete(id) {
        ...
        stopPropagation(e) {
         ...
         resetSort(e) {
            ...
            this.query.email_account_id = false;
            this.query.protocol = false;
            this.query.username = false;
...
check_one: function () {
      if ((this.query.username = false)) {
        this.query.username = true;
        this.query.type = [];
        this.query.protocol = [];
        this.query.status = [];
      }
      if ((this.query.email_account_id = false)) {
        this.query.email_account_id = true;
        this.query.username = [];
        this.query.protocol = [];
        this.query.status = [];
      }
      if ((this.query.protocol = false)) {
        this.query.protocol = true;
        this.query.username = [];
        this.query.email_account_id = [];
        this.query.status = [];
      }
      if ((this.query.status = false)) {
        ...
        loadData() {
            ...
            this.$inertia.visit(route('email-setting.index'), {
                method: 'get',
                data: {
                    ...query
                }
