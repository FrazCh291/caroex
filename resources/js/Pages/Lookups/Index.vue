  ...
  <admin-layout>
    <div class="col-12 px-0">
      <div class="col form-group p-0">
        <inertia-link :href="route('lookups.create')" class="btn btn-primary" data-repeater-create="">
          Add Lookup
        </inertia-link>
      </div>
    </div>
    <div class="row pb-3" id="table-hover-row">
                   ...
                   <div class="actions action-btns d-flex align-items-center sort-dropdown  pl-1" >
                    ...
                    <div class="dropdown w-100 pr-2 sort-dropdown2">
                        ...
                        <div class="col-12 pl-2 pt-1" id="sortmenue">
                          ...
                          <div class="align-items-center text-base pt-1">
                            <p class="tag">
                              <input type="checkbox" name="type" id="type" v-model="query.type" v-on:click="check_one()" />
                              <label class="pl-1 py-0 my-0" for="type">Type</label>
                            </p>
                          </div>
                           <div class="align-items-center text-base pt-0">
                            <p class="tag">
                              <input type="checkbox" name="value" id="value" v-model="query.value" v-on:click="check_one()" />
                              <label class="pl-1 py-0 my-0" for="value">Value</label>
                            </p>
                          </div>
                          <div class="align-items-center text-base pt-0">
                            <p class="tag">
                              ...
                              <label class="pl-1 py-0 my-0" for="slug">Slug</label>
                            </p>
                          </div>
                          <div class="align-items-center text-base pt-0">
                            <p class="tag">
                              <input type="checkbox" name="order" id="order" v-model="query.order" v-on:click="check_one()" />
                              <label class="pl-1 py-0 my-0" for="order">Order By</label>
                            </p>
                              <p class="tag">
                              <input type="checkbox" name="enable" id="enable" v-model="query.status" v-on:click="check_one()">
                              <label class="pl-1 py-0 my-0" for="enable">Status</label>
                            </p>
                          </div>
                        </div>
                        <div class="dropdown-divider py-0 my-0"></div>
          ...
          <div class="card-content">
            <div class="table-responsive">
              <table class="table table-hover mb-0">
                ...
                <tbody>
                  <tr v-for="lookup in lookups.data" >
                    ...
                    <td class="text-center">
                        <span v-if="lookup.enable == 1" class="badge badge-light-success badge-pill">Active</span>
                    ...
                    <td class="text-right py-0 my-0">
                      <div class="dropdown">
                        ...
                        <a  :href="route('lookups.edit', lookup.id)" class="dropdown-item"><i class="bx bx-edit-alt mr-1"></i>Edit</a>
                        <a class="dropdown-item"  v-on:click="confirmDelete(lookup.id)"><i class="bx bx-trash mr-1"></i>Delete</a>
                        </div>
                        </div> 
                    </td>
                  </tr>
      ...
      <div class="col-12">
        <pagination :links="lookups.links" class="float-right"></pagination>
      </div>
    </div>
  </admin-layout>
</template>
...
<script>
  ...
  export default {
    ...
    beforeMount() {
      document.title = process.env.MIX_APP_NAME + " - Lookups";
    },
    mounted() {
      if (this.params) {
    ...
    methods: {
      ...
      deleteItem() {
        this.sweetAlert = false;
        this.$inertia.delete(`/lookups/${this.itemId}`);
      },
      confirmDelete(id) {
 ...
 check_one: function () {
      ...
      loadData() {
        ...
        this.$inertia.visit(route("lookups.index"), {
          method: "get",
          data: {
            ...query,
          },
