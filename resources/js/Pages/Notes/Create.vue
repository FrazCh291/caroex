                                    ...
                                    <form @submit.prevent="submit" class="form form-horizontal">
                                        <div class="form-body">
                                            ...
                                            <div class="col-sm-11 d-flex justify-content-start px-0">
                                                ...
                                                <inertia-link :href="route('cases.show', cases.id)"
                                                              type="button"
                                                              class="btn btn-light-secondary mr-1 mb-1" v-if="cases">
                                                    Back
                                                </inertia-link>
                                                <inertia-link :href="route('cases.show', caseId.id)"
                                                              type="button"
...
export default {
    ...
    methods: {
        submit() {
            if (this.update) {
                this.form.post(route('cases.notes.update', this.notes.id))
                this.update = false;
            } else {
                this.form.case_id = this.cases.id;
                this.form.post('/erp/cases/notes/store', this.case_id);
            }
        },
