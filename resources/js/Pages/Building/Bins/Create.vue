                    ...
                    <div class="col-md-9 col-12">
                        <div class="card mb-0" >
                             <div class="card-header pb-0 mb-0">
                                     <h6 class="d-inline-block">Level: {{ level.code }} </h6><span> - </span><h6 class="d-inline-block" v-if="bin">Edit Bin</h6>
                                <h6 class="d-inline-block" v-else>Add Bin</h6>
                            </div>
                             <div class=" pb-1 px-2">
                                <hr class="line" />
                                </div>
                                    ...
                                    <form @submit.prevent="submit" class="form form-horizontal">
                                        <div class="form-body">
                                            ...
                                            <div class="row form-group pt-2 pb-0 mb-0">
                                                ...
                                                <div class="col-md-6 form-group">
                                                    <input disabled type="number" id="volume" v-model="form.volume"
...
<script>
    ...
    export default {
        name: "Create",
        props: ['bin', 'errors', 'building_code', 'level'],
        components: {
            Label,
            AdminLayout,
            JetInputError,
        ...
        beforeMount() {
            ...
            if (this.bin) {
                this.update = true;
                if (this.bin.is_empty == 1) {
                    this.bin.is_empty = 1 ? true : false;
                }
                let data = this.bin;
                Object.assign(data, {
                    _method: "PUT",
        ...
        methods: {
            submit() {
                if (this.update) {
                    this.form.post(route("bin.update", this.bin.id + "-" + this.building_code));
                } else {
                    this.form.post(route("bin.store", { 'building_code': this.building_code, 'level_id': this.level.id }));
                }
            },
        },
    };
